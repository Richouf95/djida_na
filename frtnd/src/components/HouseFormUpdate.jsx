...
function HouseForm({houseToUpdate, updateInfo}) {
    ...
    useEffect(() => {
        if(houseToUpdate) {
            setAdresse(houseToUpdate.adresse)
            setSuperficie(houseToUpdate.superficie)
            setNombreChambre(houseToUpdate.nombreChambre)
            setNombreSalon(houseToUpdate.nombreSalon)
            setCoursUnique(houseToUpdate.coursUnique)
            setDependance(houseToUpdate.dependance)
            setJardin(houseToUpdate.jardin)
            setPiscine(houseToUpdate.piscine)
            setPrix(houseToUpdate.prix)
            setDisponible(houseToUpdate.disponible)
            setImagePrincipale(houseToUpdate.imagePrincipale)
        }
    }, [houseToUpdate])
    ...
    const handleSubmit = async (e) => {
        ...
        if(response.ok) {
            ...
            setError(null)
            setEmptyFields([])
            console.log('New house added : ', house);
  ...
  return (
    <form onSubmit={handleSubmit}>
        ...
        <div className="mb-3">
            ...
            <input disabled onChange={handleImage} type="file" className={`form-control ${emptyFields && emptyFields.includes('imagePrincipale') ? 'error' : ''}`} />
        </div>
